...
module.exports = {
  read: function (email, callback) { 
    db.query(
      "select * from Candidat where email= ?", email, function (err, results) {
        if (err) throw err;
        callback(results);
      }
    );
  ...
  readall: function (callback) {
    db.query(
      "select * from Candidat", function (err, results) {
        if (err) throw err;
        callback(results);
      }
    );
  ...
  areValid: function (email, password, callback) {
    sql = "SELECT * FROM Candidat WHERE email = ?";
  ...
  delete: function(email, callback) {
    const sql = "DELETE FROM Candidat WHERE email = ?";
    db.query(sql, email, function(err, results) {
      if (err) {
  ...
  create: function(nom, prenom, email, date_creation, actif, tel, mot_de_passe, callback) {
    const sql = "INSERT INTO Candidat (nom, prenom, email, date_creation, actif, tel, mot_de_passe) VALUES (?, ?, ?, ?, ?, ?, ?)";
    db.query(
      sql,
